...
class CalendarSkill(Skill):
    ...
    @property
    def description(self) -> str: return "Calendar display, date math, business days, and countdowns"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="cal_month", description="Display a calendar for a month",
                parameters={"type":"object","properties":{
                    "year":{"type":"integer","description":"Year (default: current)"},
                    "month":{"type":"integer","description":"Month 1-12 (default: current)"},
                }}),
            ToolDefinition(name="cal_business_days", description="Count business days between two dates",
                parameters={"type":"object","properties":{
                    "start":{"type":"string","description":"Start date (YYYY-MM-DD)"},
                    "end":{"type":"string","description":"End date (YYYY-MM-DD)"},
                },"required":["start","end"]}),
            ToolDefinition(name="cal_add_days", description="Add or subtract days/weeks/months from a date",
                parameters={"type":"object","properties":{
                    "date":{"type":"string","description":"Base date (YYYY-MM-DD, default: today)"},
            ...
            ToolDefinition(name="cal_countdown", description="Countdown to a date — days, weeks, months remaining",
                parameters={"type":"object","properties":{
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "cal_month": return self._month(arguments.get("year"), arguments.get("month"))
            case "cal_business_days": return self._biz_days(arguments["start"], arguments["end"])
            case "cal_add_days": return self._add(arguments.get("date"), arguments.get("days",0), arguments.get("weeks",0), arguments.get("business_days",0))
            case "cal_countdown": return self._countdown(arguments["target"], arguments.get("label",""))
