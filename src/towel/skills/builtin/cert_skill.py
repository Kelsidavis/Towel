...
class CertSkill(Skill):
    @property
    def name(self) -> str: return "cert"
    @property
    def description(self) -> str: return "Inspect SSL/TLS certificates for any domain"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="cert_check", description="Get SSL certificate details for a domain",
                parameters={"type":"object","properties":{
                    "domain":{"type":"string","description":"Domain to check (e.g., github.com)"},
                    "port":{"type":"integer","description":"Port (default: 443)"},
                },"required":["domain"]}),
        ]

    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        if tool_name != "cert_check": return f"Unknown: {tool_name}"
        ...
        try:
            ...
            lines = [
                ...
                f"  SANs:    {', '.join(sans[:5])}" + (f" (+{len(sans)-5})" if len(sans) > 5 else ""),
