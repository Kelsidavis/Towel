...
class ClipboardHistorySkill(Skill):
    @property
    def name(self) -> str: return "clipboard_history"
    @property
    def description(self) -> str: return "Clipboard history — track and recall past clipboard contents"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="clip_capture", description="Capture current clipboard to history",
                parameters={"type":"object","properties":{}}),
            ToolDefinition(name="clip_history", description="Show clipboard history",
                parameters={"type":"object","properties":{
                    "limit":{"type":"integer","description":"Max entries (default: 10)"},
                }}),
            ToolDefinition(name="clip_recall", description="Recall a specific clipboard entry by index",
                parameters={"type":"object","properties":{
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "clip_capture": return await self._capture()
            case "clip_history": return self._show(arguments.get("limit", 10))
            case "clip_recall": return self._recall(arguments["index"])
    ...
    def _recall(self, index: int) -> str:
        if index < 0 or index >= len(_history): return f"Invalid index: {index}"
