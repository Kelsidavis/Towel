...
class CurrencySkill(Skill):
    ...
    @property
    def description(self) -> str: return "Currency exchange rates and conversion"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="currency_convert", description="Convert between currencies",
            ...
            ToolDefinition(name="currency_rates", description="Get exchange rates for a base currency",
                parameters={"type":"object","properties":{
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                if tool_name == "currency_convert":
                    ...
                    resp = await client.get(f"https://api.frankfurter.app/latest?amount={amt}&from={fr}&to={to}")
                    data = resp.json()
                    rate = data.get("rates",{}).get(to)
                    if rate: return f"{amt} {fr} = {rate} {to}"
                    ...
                    return "\n".join(lines)
        except Exception as e: return f"Currency error: {e}"
