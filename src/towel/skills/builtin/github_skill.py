...
class GithubSkill(Skill):
    ...
    @property
    def description(self) -> str: return "Search GitHub repos, view profiles, and get repo info"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="github_repo", description="Get info about a GitHub repository",
                parameters={"type":"object","properties":{
                    "repo":{"type":"string","description":"owner/repo (e.g., torvalds/linux)"},
                },"required":["repo"]}),
            ToolDefinition(name="github_search", description="Search GitHub repositories",
                parameters={"type":"object","properties":{
                    "query":{"type":"string","description":"Search query"},
                    "limit":{"type":"integer","description":"Max results (default: 5)"},
                },"required":["query"]}),
            ToolDefinition(name="github_user", description="Get GitHub user/org profile info",
                parameters={"type":"object","properties":{
        ...
        try:
            async with httpx.AsyncClient(timeout=10, headers=headers) as client:
                if tool_name == "github_repo":
                    ...
                    if resp.status_code == 404: return f"Repo not found: {arguments['repo']}"
                    r = resp.json()
                    return (f"{r['full_name']} — {r.get('description','')}\n"
                            f"  Stars: {r['stargazers_count']:,}  Forks: {r['forks_count']:,}\n"
                            f"  Language: {r.get('language','?')}  License: {(r.get('license') or {}).get('spdx_id','?')}\n"
                    ...
                    lines = []
                    for r in items:
                        lines.append(f"  {r['full_name']} ★{r['stargazers_count']:,} — {r.get('description','')[:60]}")
                    ...
                    if resp.status_code == 404: return f"User not found: {arguments['username']}"
                    u = resp.json()
                    return (f"{u.get('name',u['login'])} (@{u['login']})\n"
                            f"  {u.get('bio','')}\n"
