...
_CHECKS = [
    ...
    ("hardcoded_ip", re.compile(r"\b(?:192\.168|10\.\d+|172\.(?:1[6-9]|2\d|3[01]))\.\d+\.\d+\b"), "Hardcoded private IP"),
...
class LintSkill(Skill):
    @property
    def name(self) -> str: return "lint"
    @property
    def description(self) -> str: return "Basic code style and quality checks without external tools"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="lint_file", description="Run basic lint checks on a file",
                parameters={"type":"object","properties":{
                    "path":{"type":"string","description":"File to check"},
                    "checks":{"type":"array","items":{"type":"string"},
                              "description":"Specific checks (default: all)"},
                },"required":["path"]}),
            ToolDefinition(name="lint_text", description="Run lint checks on text/code directly",
                parameters={"type":"object","properties":{
                    "code":{"type":"string","description":"Code to check"},
                    "language":{"type":"string","description":"Language hint (python, javascript, etc.)"},
                },"required":["code"]}),
        ]

    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "lint_file": return self._lint_file(arguments["path"], arguments.get("checks"))
            case "lint_text": return self._lint_text(arguments["code"], arguments.get("language",""))
    ...
    def _run_checks(self, code: str, name: str, filter_checks: list[str]|None) -> str:
        ...
        for check_name, pattern, desc in _CHECKS:
            if filter_checks and check_name not in filter_checks: continue
