...
class NpmRegistrySkill(Skill):
    @property
    def name(self) -> str: return "npm_registry"
    @property
    def description(self) -> str: return "Search npm registry and get package info"
    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="npm_search", description="Search npm for packages",
                parameters={"type":"object","properties":{"query":{"type":"string"},"limit":{"type":"integer"}},"required":["query"]}),
            ToolDefinition(name="npm_pkg_info", description="Get info about an npm package",
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as c:
                if tool_name == "npm_search":
                    ...
                    for p in pkgs:
                        pkg = p.get("package",{})
                        lines.append(f"  {pkg.get('name','?')} v{pkg.get('version','?')} — {pkg.get('description','')[:60]}")
                    return "\n".join(lines)
                elif tool_name == "npm_pkg_info":
                    resp = await c.get(f"https://registry.npmjs.org/{arguments['package']}/latest")
                    if resp.status_code == 404: return f"Not found: {arguments['package']}"
                    d = resp.json()
                    return (f"{d.get('name','?')} v{d.get('version','?')}\n"
                            f"  {d.get('description','')}\n"
                            f"  License: {d.get('license','?')}\n"
                            f"  Deps: {len(d.get('dependencies',{}))}\n"
                            f"  Homepage: {d.get('homepage','?')}")
        except Exception as e: return f"npm error: {e}"
