...
class PdfSkill(Skill):
    @property
    def name(self) -> str: return "pdf"
    @property
    def description(self) -> str: return "Extract text and metadata from PDF files (basic, no dependencies)"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="pdf_info", description="Get PDF metadata — pages, title, author, version",
                parameters={"type":"object","properties":{
                    "path":{"type":"string","description":"Path to PDF file"},
                },"required":["path"]}),
            ToolDefinition(name="pdf_text", description="Extract text content from a PDF (basic extraction)",
                parameters={"type":"object","properties":{
                    "path":{"type":"string","description":"Path to PDF file"},
                    "max_chars":{"type":"integer","description":"Max characters to extract (default: 10000)"},
                },"required":["path"]}),
        ]

    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "pdf_info": return self._info(arguments["path"])
            case "pdf_text": return self._text(arguments["path"], arguments.get("max_chars", 10000))
    ...
    def _info(self, path: str) -> str:
        ...
        for k, v in meta.items():
            lines.append(f"  {k}: {v}")
        if not meta: lines.append("  (no metadata extracted)")
        return "\n".join(lines)
