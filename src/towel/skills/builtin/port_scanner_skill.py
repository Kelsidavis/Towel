...
class PortScannerSkill(Skill):
    ...
    @property
    def description(self) -> str: return "Scan common ports on a host"
    def tools(self) -> list[ToolDefinition]:
        return [ToolDefinition(name="scan_ports", description="Scan common ports on a host",
            parameters={"type":"object","properties":{"host":{"type":"string"},
                "ports":{"type":"array","items":{"type":"integer"},"description":"Custom ports (default: common ports)"}},"required":["host"]})]
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        if tool_name != "scan_ports": return f"Unknown: {tool_name}"
        host = arguments["host"]
