...
class PypiSkill(Skill):
    @property
    def name(self) -> str: return "pypi"
    @property
    def description(self) -> str: return "Search PyPI packages and get version/dependency info"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="pypi_info", description="Get info about a PyPI package",
                parameters={"type":"object","properties":{
                    "package":{"type":"string","description":"Package name"},
                },"required":["package"]}),
            ToolDefinition(name="pypi_search", description="Search for packages on PyPI",
                parameters={"type":"object","properties":{
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                if tool_name == "pypi_info":
                    ...
                    if resp.status_code == 404: return f"Package not found: {pkg}"
                    info = resp.json()["info"]
                    ...
                    import re
                    packages = re.findall(r'class="package-snippet__name">([^<]+)</span>\s*<span[^>]*>([^<]+)</span>',
                                         resp.text)
                    if not packages: return "No packages found (PyPI search is limited)."
                    lines = [f"PyPI search: {arguments['query']}"]
                    for name, version in packages[:10]:
                        lines.append(f"  {name.strip()} {version.strip()}")
                    return "\n".join(lines)
        except Exception as e: return f"PyPI error: {e}"
