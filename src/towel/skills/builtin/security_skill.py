...
SKIP_EXT = {".pyc",".pyo",".so",".dylib",".o",".exe",".bin",".zip",".tar",".gz",".png",".jpg",".gif"}


class SecuritySkill(Skill):
    @property
    def name(self) -> str: return "security"
    @property
    def description(self) -> str: return "Security scanning — find hardcoded secrets, audit file permissions"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="scan_secrets", description="Scan files for hardcoded secrets, API keys, and tokens",
                parameters={"type":"object","properties":{
                    "path":{"type":"string","description":"Directory or file to scan (default: cwd)"},
                    "glob":{"type":"string","description":"File filter (e.g., '*.py', '*.env')"},
                },"required":[]}),
            ToolDefinition(name="check_permissions", description="Check file permissions for security issues (world-writable, etc.)",
                parameters={"type":"object","properties":{
                    "path":{"type":"string","description":"Directory to check (default: cwd)"},
                },"required":[]}),
            ToolDefinition(name="scan_dependencies", description="Check for known patterns in dependency files (requirements.txt, package.json)",
                parameters={"type":"object","properties":{
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "scan_secrets": return self._scan_secrets(arguments.get("path","."), arguments.get("glob"))
            case "check_permissions": return self._check_perms(arguments.get("path","."))
            case "scan_dependencies": return self._scan_deps(arguments.get("path","."))
    ...
    def _scan_secrets(self, path: str, file_glob: str|None) -> str:
        ...
        for fp in files:
            ...
            if fp.suffix in SKIP_EXT: continue
            try:
    ...
    def _check_perms(self, path: str) -> str:
        ...
        if not issues: return "No permission issues found."
    ...
    def _scan_deps(self, path: str) -> str:
        ...
        if req.exists():
            ...
            unpinned = [l.strip() for l in content.splitlines() if l.strip() and not l.startswith("#") and "==" not in l and ">=" not in l]
            if unpinned:
                lines.append(f"requirements.txt: {len(unpinned)} unpinned dependencies")
                for d in unpinned[:10]: lines.append(f"    {d}")
        # Check package.json
        ...
        if pkg.exists():
            try:
                ...
                if stars:
                    ...
                    for s in stars[:10]: lines.append(f"    {s}")
            except: pass
        if not lines: return "No dependency issues found."
        return "\n".join(lines)
