...
def _load() -> list[dict]:
    ...
    try: return json.loads(TODO_FILE.read_text(encoding="utf-8"))
    except: return []
...
class TodoSkill(Skill):
    ...
    @property
    def description(self) -> str: return "Task management — add, complete, list, and prioritize todos"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(name="todo_add", description="Add a new todo item",
            ...
            ToolDefinition(name="todo_list", description="List all todos, optionally filtered",
                parameters={"type":"object","properties":{
                    "show_done":{"type":"boolean","description":"Include completed (default: false)"},
                    "priority":{"type":"string","description":"Filter by priority"},
                    "tag":{"type":"string","description":"Filter by tag"},
                }}),
            ToolDefinition(name="todo_done", description="Mark a todo as completed by index",
                parameters={"type":"object","properties":{
                    "index":{"type":"integer","description":"Todo index"},
                },"required":["index"]}),
            ToolDefinition(name="todo_remove", description="Delete a todo by index",
                parameters={"type":"object","properties":{
                    "index":{"type":"integer","description":"Todo index"},
                },"required":["index"]}),
        ]
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "todo_add": return self._add(arguments["task"], arguments.get("priority","medium"), arguments.get("due"), arguments.get("tags",[]))
            case "todo_list": return self._list(arguments.get("show_done",False), arguments.get("priority"), arguments.get("tag"))
            case "todo_done": return self._done(arguments["index"])
            case "todo_remove": return self._remove(arguments["index"])
    ...
    def _list(self, show_done: bool, priority: str|None, tag: str|None) -> str:
        ...
        for i, t in enumerate(todos):
            if not show_done and t.get("done"): continue
            if priority and t.get("priority") != priority: continue
            if tag and tag.lower() not in t.get("tags",[]): continue
            filtered.append((i, t))
        filtered.sort(key=lambda x: pri_order.get(x[1].get("priority","medium"), 1))
        if not filtered: return "No todos." + (" (use show_done=true to see completed)" if not show_done else "")
