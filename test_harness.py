...
def test_basic_response() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-basic")
...
def test_read_file() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-read")
...
def test_list_directory() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-listdir")
...
def test_run_command() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-cmd")
...
def test_current_time() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-time")
    # Should contain something time-like (hour:minute pattern)
    import re
...
def test_date_now() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-date")
...
def test_system_info() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-sysinfo")
...
def test_git_status() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-git")
...
def test_search_files() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-search")
...
def test_todo() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-todo")
...
def test_json_skill() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-json")
    # Should contain flattened keys
    has_flat = "a.b" in resp or "a.c.d" in resp or '"b"' in resp
...
def test_hash_text() -> TestResult:
    ...
    prompt = "Compute the MD5 hash of the string 'hello'."
    resp, elapsed = ask(prompt, session="test-hash")
    # MD5 of "hello" is 5d41402abc4b2a76b9719d911017c592
    passed, failed = check(resp, "5d41402abc4b2a76b9719d911017c592")
...
def test_memory() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-memory")
...
def test_no_hallucinated_tools() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-nohalluc")
    passed, failed = check(resp, "blue")
    # Also check it didn't try to call a weird tool
...
def test_multi_step() -> TestResult:
    ...
    resp, elapsed = ask(prompt, session="test-multistep")
